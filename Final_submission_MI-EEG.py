...
eeg_events = mne.events_from_annotations(eeg_data, event_id=events_mapping)

# Creating events from the annotations
mne.Epochs(eeg_data, eeg_events[0], tmin=-0.2, tmax=0.8, event_repeated='merge',preload=True, event_id={'left':769, 'right': 770, 'foot': 771, 'tongue':772})
...
# filtering raw EEG data (4Hz,30Hz). Considering theta, mu/alpha, beta bands
filtered_eeg_data = eeg_data.copy()
filtered_eeg_data.filter(l_freq=4, h_freq=40)

# resample the filtered data to 100Hz
...
eeg_epochs = mne.Epochs(filtered_eeg_data, eeg_events[0], tmin=-0.2, tmax=0.8, event_repeated='merge',preload=True, event_id={'left':769, 'right': 770, 'foot': 771, 'tongue':772})
...
eeg_epochs.get_data().shape[2]

signals = eeg_epochs.get_data()
signals.shape
...
def preprocess(path, subject_id):
  ...
  eeg_epochs = mne.Epochs(filtered_eeg_data, eeg_events1[0], tmin=start_time, tmax=end_time, event_repeated='merge',preload=True, event_id={'left':769, 'right': 770, 'foot': 771, 'tongue':772})
  #print("epochs information: ", eeg_epochs)
  #print("epochs dropped for: ", set(eeg_epochs.drop_log))
...
a,b = preprocess(directory_path,1)
...
unique_elements, counts = np.unique(test_label_array, return_counts = True)
...
train_data_array, test_data_array, train_label_array, test_label_array = train_test_split(data_array, label_array, test_size=0.15, random_state=42, shuffle=True)
